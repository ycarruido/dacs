...
const GlobeWithOrbitingWindows = () => {
  ...
  useEffect(() => {
    ...
    resizeRenderer();
    // Ajustar la posición de la cámara para móviles y mantener el comportamiento en pantallas grandes
    camera.position.z = 6; // Mantener el comportamiento normal en pantallas grandes
    const ambientLight = new THREE.AmbientLight(0xffffff, 1);
    scene.add(ambientLight);
    const globeRadius = 2; // Mantener el tamaño del globo igual en pantallas grandes
    ...
    const orbitRadius = isMobile ? 2.5 : 2.8; // Ajustar el radio de órbita en móviles
    const projects = [
    ...
    projects.forEach((project, index) => {
      ...
      createPanelTexture(project, (texture) => {
        ...
        // Inclinación hacia atrás en móviles
        if (isMobile) {
          plane.rotation.x = -0.3 // Ajusta este valor para cambiar el ángulo de inclinación
          plane.position.y += 0.3; // Elevar la posición de las ventanas para que no choquen con el globo
        }
    ...
    const animate = () => {
      ...
      if (animProgress < animDuration) {
        camera.position.z = 6 - (animProgress / animDuration) * 2; // Mantener el comportamiento original en pantallas grandes
      ...
      orbitObjects.forEach((obj) => {
        obj.angle += isMobile ? 0.005 : 0.002; // Aumentar la velocidad de órbita en móviles
